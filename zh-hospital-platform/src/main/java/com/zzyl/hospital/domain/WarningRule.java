package com.zzyl.hospital.domain;

import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import lombok.AllArgsConstructor;
import lombok.Data;
import lombok.NoArgsConstructor;
import com.zzyl.common.annotation.Excel;
import com.zzyl.common.core.domain.BaseEntity;

/**
 * 预警规则对象 warning_rule
 * 
 * @author alexis
 * @date 2025-07-14
 */
@Data
@NoArgsConstructor
@AllArgsConstructor
@ApiModel(value="WarningRule对象", description="预警规则")
public class WarningRule extends BaseEntity
{
    private static final long serialVersionUID = 1L;

    /** 主键ID */
    @ApiModelProperty("主键ID")
    private Long id;

    /** 所属产品的key */
    @Excel(name = "所属产品的key")
    @ApiModelProperty("所属产品的key")
    private String productKey;

    /** 产品名称 */
    @Excel(name = "产品名称")
    @ApiModelProperty("产品名称")
    private String productName;

    /** 模块的key */
    @Excel(name = "模块的key")
    @ApiModelProperty("模块的key")
    private String moduleId;

    /** 模块名称 */
    @Excel(name = "模块名称")
    @ApiModelProperty("模块名称")
    private String moduleName;

    /** 功能名称 */
    @Excel(name = "功能名称")
    @ApiModelProperty("功能名称")
    private String functionName;

    /** 功能标识 */
    @Excel(name = "功能标识")
    @ApiModelProperty("功能标识")
    private String functionId;

    /** 物联网设备id */
    @Excel(name = "物联网设备id")
    @ApiModelProperty("物联网设备id")
    private String iotId;

    /** 设备名称 */
    @Excel(name = "设备名称")
    @ApiModelProperty("设备名称")
    private String monitoringDeviceName;

    /** 预警数据类型，0：患者异常数据，1：设备异常数据 */
    @Excel(name = "预警数据类型，0：患者异常数据，1：设备异常数据")
    @ApiModelProperty("预警数据类型，0：患者异常数据，1：设备异常数据")
    private Integer warningDataType;

    /** 告警规则名称 */
    @Excel(name = "告警规则名称")
    @ApiModelProperty("告警规则名称")
    private String warningRuleName;

    /** 运算符 */
    @Excel(name = "运算符")
    @ApiModelProperty("运算符")
    private String operator;

    /** 阈值 */
    @Excel(name = "阈值")
    @ApiModelProperty("阈值")
    private Double value;

    /** 持续周期 */
    @Excel(name = "持续周期")
    @ApiModelProperty("持续周期")
    private Integer duration;

    /** 预警生效时段 */
    @Excel(name = "预警生效时段")
    @ApiModelProperty("预警生效时段")
    private String warningEffectivePeriod;

    /** 预警沉默周期 */
    @Excel(name = "预警沉默周期")
    @ApiModelProperty("预警沉默周期")
    private Integer warningSilentPeriod;

    /** 0 禁用 1启用 */
    @Excel(name = "0 禁用 1启用")
    @ApiModelProperty("0 禁用 1启用")
    private Integer status;

}
