...
public class HealthAssessmentServiceImpl extends ServiceImpl<HealthAssessmentMapper, HealthAssessment> implements IHealthAssessmentService {
    ...
    public Long insertHealthAssessment(HealthAssessment healthAssessment) {
        try {
            ...
            String prompt = getPrompt(healthAssessment);
            //2.调用AI模型进行预测
     ...
     * @return prompt
     */
    private String getPrompt(HealthAssessment healthAssessment) {
        //1.从缓存中获取prompt模板
        String content = (String) redisTemplate.opsForHash().get("healthReport", healthAssessment.getIdCard());
        // 判断是否为空
