-- Flyway V4: 表/列注释更新 + 主题数据转换
-- 来源：sql/20260421_hospital_patient_theme.sql + sql/20260422_hospital_schema_refactor.sql
-- 说明：注释更新幂等（ALTER TABLE COMMENT / MODIFY COLUMN 始终覆盖旧值）
--       数据更新使用 WHERE 条件精确匹配，重复执行无副作用
